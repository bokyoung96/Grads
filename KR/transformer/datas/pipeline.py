...
from ..core.params import TransformerConfig
    ...
    key = json.dumps(
        {
            ...
            "norm": cfg.norm,
            "use_univ": bool(cfg.use_univ),
            "universe_path": str(cfg.universe_path),
        },
        sort_keys=True,
    ).encode("utf-8")
...
def build_index(cfg: TransformerConfig, *, panel) -> WindowIndex:
    ...
    pbar = tqdm(range(0, max_start + 1, cfg.stride), desc="mfd windows", leave=False)
    for s in pbar:
        e = s + cfg.lookback
        t_idx = e - 1
        ...
        idx = np.where(ok)[0]
        ratio = float(getattr(cfg, "min_assets_ratio", 0.25))
        floor = int(getattr(cfg, "min_assets_floor", 190))
        if panel.univ_counts is not None:
            n_univ = int(panel.univ_counts[t_idx])
            min_assets_dyn = max(int(cfg.min_assets), int(np.ceil(n_univ * ratio)))
            min_assets_dyn = max(floor, min_assets_dyn)
            min_assets_dyn = min(min_assets_dyn, max(n_univ, 1))
        else:
            n_univ = int(n_assets)
            min_assets_dyn = max(int(cfg.min_assets), floor)
        if idx.size < min_assets_dyn:
            continue
        n_valid = idx.size
        log_dates.append(panel.dates[t_idx].to_datetime64())
        log_univ.append(int(n_univ))
        log_valid.append(int(n_valid))
        log_min_assets.append(int(min_assets_dyn))
        pbar.set_postfix({"valid": int(n_valid), "univ": int(n_univ), "min": int(min_assets_dyn)})
    ...
    if log_dates:
        stats = pd.DataFrame(
            {
                "date": pd.to_datetime(log_dates),
                "universe": log_univ,
                "valid": log_valid,
                "min_assets": log_min_assets,
            }
        )
        summary = stats.groupby("date", as_index=False).agg(
            universe=("universe", "max"),
            valid=("valid", "sum"),
            min_assets=("min_assets", "max"),
        )
        logger = logging.getLogger(__name__)
        valid_arr = np.asarray(summary["valid"], dtype=np.float64)
        univ_arr = np.asarray(summary["universe"], dtype=np.float64)
        valid_min = int(valid_arr.min())
        valid_med = float(np.median(valid_arr))
        valid_max = int(valid_arr.max())
        univ_min = int(univ_arr.min())
        univ_med = float(np.median(univ_arr))
        univ_max = int(univ_arr.max())
        logger.warning(
            "window stats: dates=%d valid_min=%d valid_med=%.1f valid_max=%d univ_min=%d univ_med=%.1f univ_max=%d",
            len(summary),
            valid_min,
            valid_med,
            valid_max,
