...
def _sample_univ_mask(idx, *, panel, path: Path = UNIVERSE_PARQUET, align_to: str = "label") -> np.ndarray:
    ...
    if align_to == "label":
        target_dates = idx.dates
    elif align_to == "base":
        # base date = target date - horizon; horizon is encoded in idx.start vs idx.dates positions
        # idx.start points to panel start positions; base date is panel.dates[idx.start + lookback - 1]
        base_pos = idx.start + (0)
        target_dates = dates[base_pos]
    else:
        raise ValueError("align_to must be 'label' or 'base'")

    pos = np.searchsorted(dates, target_dates)
    ok = (pos >= 0) & (pos < len(dates)) & (dates[pos] == target_dates)
...
def _train_one(
    ...
    *,
    ...
    if cfg.use_univ:
        ...
        # align_to="base" avoids lookahead: use base-date (lookback end) universe membership
        univ_ok = _sample_univ_mask(bundle.idx, panel=panel, path=cfg.universe_path, align_to="base")
        train_mask &= univ_ok
        test_mask &= univ_ok
        logger.info(
...
def train(
    *,
    ...
    try:
        ...
        if cfg_path is None and not use_bm:
            fallback = TRANSFORMER_DIR / "config" / "config_bm.json"
            logger.warning("Missing features at %s; retrying with BM config %s", cfg.features_dir, fallback)
            params = TransformerParams(config_path=fallback)
            cfg = params.get_config(mode=mode, timeframe=timeframe)
            params.validate_features(cfg.features, use_bm=cfg.use_bm)
            bundle = load_bundle(cfg)
        else:
